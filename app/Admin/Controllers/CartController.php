...
class CartController extends AdminController
{
    ...
    {
        $grid = new Grid(new CartgoryModel());
    ...
    {
        $show = new Show(CartgoryModel::findOrFail($id));
...
//        $show->field('float_percent', __('Float percent'));
    ...
    {
        $form = new Form(new CartgoryModel());

        $form->text('cat_name', __('Cat name'));
        $form->text('keywords', __('Keywords'));
        $form->text('cat_desc', __('Cat desc'));
        $form->select('parent_id', __('Parent id'))->options(CartgoryModel::selectOptions());
